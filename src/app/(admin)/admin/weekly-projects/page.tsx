        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Manage published, draft, and archived weekly drives.</p>
                </div>
                <Button asChild>
            ...
            <div className="grid gap-4">
                {projects.map((project) => {
                    const collected = project.donations.reduce((sum, item) => sum + item.amount, 0);
                    return (
                        <Card key={project.id}>
                            <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
                                ...
                                <Badge variant={statusClass[project.status] ?? "outline"}>{project.status}</Badge>
                            </CardHeader>
                            <CardContent className="flex flex-wrap items-center gap-6 text-sm">
                                <p>
                                    Target: <span className="font-semibold">{project.targetAmount.toLocaleString()} BDT</span>
                                </p>
                                <p>
                                    Current: <span className="font-semibold">{collected.toLocaleString()} BDT</span>
                                </p>
                                <p>
                                    Duration: <span className="font-semibold">{project.startDate ? project.startDate.toLocaleDateString() : "-"}</span> -{" "}
                                    <span className="font-semibold">{project.endDate ? project.endDate.toLocaleDateString() : "-"}</span>
                                </p>
