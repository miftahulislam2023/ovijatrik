    ...
    return (
        <div className="space-y-6">
            ...
            <form className="grid gap-3 rounded-lg border border-border p-3 md:grid-cols-[1fr_180px_auto]" method="get">
                <input name="q" defaultValue={q} placeholder="Search sender, subject, message" className="rounded-md border border-input px-3 py-2" />
                <select name="read" defaultValue={read} className="rounded-md border border-input px-3 py-2">
                ...
                {messages.map((message) => (
                    <Card key={message.id} className="transition hover:border-primary">
                        ...
                        <CardContent className="flex items-center justify-between gap-3 text-sm text-muted-foreground">
                            <p>{message.name} - {message.email} {message.readAt ? "(Read)" : "(Unread)"}</p>
