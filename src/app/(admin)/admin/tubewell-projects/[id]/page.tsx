...
export default async function EditTubewellProjectPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function updateAction(formData: FormData) {
        ...
        const completionDate = completionDateStr ? new Date(completionDateStr) : project.completionDate;
            ...
            <CardContent>
                <form action={updateAction} className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <input name="location" defaultValue={project.location} className="rounded-md border border-input px-3 py-2" required />
                        <input
                            name="completionDate"
                            type="date"
                    ...
                    <textarea name="descriptionBn" defaultValue={project.descriptionBn} rows={5} className="w-full rounded-md border border-input px-3 py-2" required />
                    <textarea name="descriptionEn" defaultValue={project.descriptionEn ?? ""} rows={5} className="w-full rounded-md border border-input px-3 py-2" />
                    <textarea name="impactSummary" defaultValue={project.impactSummary ?? ""} rows={4} className="w-full rounded-md border border-input px-3 py-2" />
                    <textarea name="photoUrls" defaultValue={project.photos.join("\n")} rows={4} className="w-full rounded-md border border-input px-3 py-2" />
