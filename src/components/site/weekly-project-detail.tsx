...
type WeeklyProjectDetailData = {
  ...
  targetAmount: number;
  photos: string[];
  donations: WeeklyProjectDonation[];
};
...
export function WeeklyProjectDetail({
  project,
  language,
}: {
  project: WeeklyProjectDetailData;
  language: "en" | "bn";
}) {
  const copy = {
    en: {
  ...
  const total = project.donations.reduce(
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:px-8">
        ...
        <article>
          {shouldRenderDescriptionHtml ? (
            <div
              className="prose prose-slate max-w-none text-foreground/85 dark:prose-invert"
              dangerouslySetInnerHTML={{ __html: safeDescriptionHtml }}
            />
          ) : (
            <p className="text-base leading-8 text-foreground/85">{description}</p>
          )}
                ...
                >
                  <div>
                    <p className="font-medium text-slate-800">
                      {donation.donorName || content.anonymous}{" "}
                      <span className="text-xs text-slate-500">
                        ({maskPhone(donation.phone || undefined)})
                      </span>
                    </p>
                    {donation.comments ? (
                  ...
                  <p className="text-sm font-semibold text-primary">
                    {donation.amount.toLocaleString()} {content.currency}
                  </p>
                </div>
              ))}
            </div>
